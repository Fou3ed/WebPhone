...
Groups.getAllGroups = (id, offset, result) => {
    ...
    dbPool.query('SELECT  G.*,L.user_id FROM webphone.groups G INNER JOIN webphone.logs L ON L.user_id=? AND L.action="POST/groups/create/" AND L.element_id=G.id LIMIT 10 offset ? ', [id, Number(offset)], (error, res) => {

        if (!error) {

            dbPool.query('SELECT count(*) as total  FROM webphone.groups G INNER JOIN webphone.logs L ON L.user_id=? AND L.action="POST/groups/create/"  AND L.element_id=G.id', [id], (error, res2) => {
...
Groups.getAllGroupsByClass = (id, classs, offset, result) => {
    ...
    dbPool.query('SELECT distinct G.*,L.user_id FROM webphone.groups G INNER JOIN webphone.logs L ON L.user_id=? AND L.action="POST/groups/create/" AND G.class=? AND L.element_id=G.id LIMIT 10 offset ? ', [id, classs, Number(offset)], (error, res) => {

        if (!error) {

            dbPool.query('SELECT count(*) as total FROM webphone.groups G INNER JOIN webphone.logs L ON L.user_id=? AND L.action="POST/groups/create/" AND G.class=? AND L.element_id=G.id', [id, classs], (error, res2) => {
